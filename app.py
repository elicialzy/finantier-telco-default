...
def predict():
    ...
    query_df = query_df.drop(columns=['PhoneService', 'gender', 'MultipleLines', 'Dependents', 'Partner', 'SeniorCitizen'])
    ...
    add_x_train = query_df[['tenure', 'MonthlyCharges']].reset_index(drop=True)
    ...
    # Predict results
    prediction = model.predict(query.to_numpy())
